...
- [x] **`nd-theme` activado y verificado en un sitio real por primera vez en toda la vida del proyecto** — toda la verificación en navegador de sesiones anteriores fue contra `wp-admin`, independiente del tema activo. Reveló el bug más significativo de esta versión: `Application::boot()` arrancaba en `plugins_loaded`, antes de que el tema activo tuviera ocasión de registrarse en el filtro `nd_core/providers` (que ocurre al cargar su `functions.php`, después de `plugins_loaded`) — `NDTheme\Providers\ThemeServiceProvider` nunca se incluía en ningún sitio real desde alpha.2, así que ni sus assets (CSS/JS de Vite) ni sus theme supports/menús se registraban jamás. Corregido moviendo el arranque a `after_setup_theme` (prioridad 5). Confirmado en vivo: assets cargando, modo oscuro funcionando, y los meta tags/JSON-LD/breadcrumbs de nd-seo (dados por completos desde alpha.3) funcionando en un artículo real por primera vez. De paso se encontró y corrigió un `comments.php` faltante (aviso de obsolescencia de WordPress en cada artículo).
