...
final class EditorialNoteRepository {
	...
	public function forPost( int $postId ): array {
		...
		$rows  = $this->db->select( "SELECT * FROM {$table} WHERE post_id = %d ORDER BY created_at DESC", array( $postId ) );

		return array_map( $this->hydrate( ... ), $rows );
	}

	public function delete( int $noteId ): bool {
		return (bool) $this->db->delete( $this->db->table( self::TABLE ), array( 'id' => $noteId ) );
	}
