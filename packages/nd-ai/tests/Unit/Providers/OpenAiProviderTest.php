...
final class OpenAiProviderTest extends BrainMonkeyTestCase
{
    ...
    public function test_returns_the_completion_text_on_success(): void
    {
        ...
        Functions\expect('wp_remote_retrieve_body')->once()->andReturn((string) json_encode([
            'choices' => [
                ['message' => ['content' => '  Titular generado  ']],
            ],
        ]));
        Functions\expect('wp_remote_retrieve_headers')->once()->andReturn([]);
    ...
    public function test_throws_when_the_response_has_no_text_content(): void
    {
        Functions\expect('is_wp_error')->once()->andReturn(false);
        Functions\expect('wp_remote_retrieve_response_code')->once()->andReturn(200);
        Functions\expect('wp_remote_retrieve_body')->once()->andReturn((string) json_encode(['choices' => []]));
        Functions\expect('wp_remote_retrieve_headers')->once()->andReturn([]);
        Functions\expect('wp_remote_request')->once()->andReturn([]);
        Functions\when('wp_json_encode')->alias('json_encode');
