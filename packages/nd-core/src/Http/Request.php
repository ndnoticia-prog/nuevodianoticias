...
final class Request {
	 ...
	 * @param array<string, string> $headers
	 */
	public static function json( string $method, string $url, array $data = array(), array $headers = array() ): self {
		return new self(
			method: $method,
			url: $url,
			headers: $headers + array( 'Content-Type' => 'application/json' ),
			body: $data === array() ? '' : (string) wp_json_encode( $data, JSON_THROW_ON_ERROR ),
		);
	}
