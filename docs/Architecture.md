...
## Menú de admin compartido: mismo patrón que las rutas REST

Antes de v0.1.0-beta.1, cada paquete solo exponía datos vía REST; construir una interfaz visual para seis paquetes distintos sin acoplarlos entre sí necesitaba el mismo patrón ya usado para las rutas REST (`nd_core/rest_controllers` + `RestApiServiceProvider`, ver "Extender la API desde un paquete" en `API.md`): un filtro (`nd_core/admin_pages`), un contrato (`NDCore\Admin\Contracts\RegistersAdminPages`) y un value object (`NDCore\Admin\AdminPage`), centralizados por `NDCore\Providers\AdminMenuServiceProvider`.

Cada paquete que quiere una página en el menú "ND Platform" implementa `RegistersAdminPages::adminPages(): array` y se engancha al filtro en su propio `ServiceProvider::boot()` — igual que un `RestController` se engancha a `nd_core/rest_controllers`. `AdminMenuServiceProvider` recoge todas las páginas de todos los paquetes activos, las ordena por `position`, y usa la de `position` más bajo para fijar el slug del menú de nivel superior (`add_menu_page()`) en vez de crear una página "índice" separada — el mismo patrón que WooCommerce o Yoast SEO, que evita un submenú duplicado/huérfano.

**Lección real de esta versión**: `AdminMenuServiceProvider` estuvo completamente implementado y probado unitariamente, pero nunca se añadió a `Application::resolveProviderClasses()` — el menú simplemente no aparecía en `wp-admin`, y ningún test (unitario ni de integración) lo detectó, porque ninguno ejercitaba la cadena completa `Application::boot()` → menú realmente registrado. Solo la verificación manual en un navegador contra un WordPress real lo reveló. Por eso el proceso de esta versión exigió, para cada página nueva, no solo `composer run check` en verde sino confirmar la página funcionando en `wp-admin` antes de darla por terminada.

**Assets de admin en desarrollo (repositorios `path` symlinked)**: `NDCore\Support\AssetUrl::for(string $rutaAbsoluta)` resuelve una URL a partir de `__DIR__`, pero dentro de un paquete hermano instalado como repositorio `path` de Composer (symlink en `vendor/`), `__DIR__` de PHP resuelve la ruta *canónica real* del symlink, no la ruta tal como se alcanza a través de `vendor/`, así que la URL calculada quedaba vacía y el CSS/JS de las páginas de admin nunca se encolaba. `AssetUrl::forPackage(string $paqueteComposer, string $rutaRelativa)` evita el problema por completo: construye la URL a partir del nombre de paquete Composer *conocido* (una cadena literal), sin introspeccionar el filesystem.

## Infraestructura de pruebas de integración con WordPress real

Todos los paquetes documentaban desde alpha.1 clases "no cubribles de forma fiable con Brain Monkey" (`WP_Query`, `WP_Post`, `$wpdb` reales). v0.1.0-beta.1 añade esa cobertura sin Docker/`wp-env` (no disponible en este entorno): `git sparse-checkout` de `WordPress/wordpress-develop` para el arnés de pruebas oficial, y MariaDB instalada sin `sudo` vía un Homebrew de usuario — ver `tools/wp-tests/README.md` para el paso a paso completo.

**PHPUnit 9 aislado y compartido**: el arnés de pruebas de `wordpress-develop` (`WP_UnitTestCase::expectDeprecated()`) todavía llama a un método eliminado en PHPUnit 11, la versión que usan las pruebas unitarias (Brain Monkey) de todos los paquetes. En vez de fijar todo el monorepo a PHPUnit 9, las pruebas de integración corren en un proceso completamente aislado (`tools/wp-tests/phpunit9/`, un "meta-proyecto" Composer propio) cuyo autoloader mapea el namespace de cada paquete directamente a su `src/` por ruta — deliberadamente sin cargar el `vendor/autoload.php` de ningún paquete individual, que arrastraría PHPUnit 11 al mismo proceso y produciría un choque de clases.

**Paquetes "librería" se activan cargando `nd-core.php`**: solo `nd-core` (plugin) y `nd-theme` (tema) tienen un punto de entrada propio que WordPress carga directamente. El resto de paquetes (`nd-search`, `nd-ads`, `nd-workflow`, ...) son librerías puras cuyas clases viajan dentro del `vendor/` de `nd-core` (ver "Dependencias entre paquetes" más arriba) — activarlas para una prueba de integración no requiere ningún truco especial: basta con requerir `nd-core.php` vía `tests_add_filter('muplugins_loaded', ...)`, exactamente igual que en producción, porque `Application::resolveProviderClasses()` ya registra el `ServiceProvider` de cada paquete instalado con un simple `class_exists()`.

**Migraciones y estado compartido entre invocaciones del proceso**: `NDCore\Providers\CoreServiceProvider::maybeRunUpgrade()` (enganchado a `init`) ejecuta `Installer::install()` automáticamente en cada arranque si `installed_version` no coincide con la versión actual — exactamente igual que en un sitio real. Esto significa que las tablas propias de cada paquete **ya existen** para cuando una clase de prueba arranca; una clase de prueba que las cree/destruya en `wpSetUpBeforeClass`/`wpTearDownAfterClass` rompe esa invariante, porque `nd_migrations`/`installed_version` (que persisten en la base de datos `wordpress_test` compartida entre invocaciones separadas del proceso de PHPUnit, a diferencia de las filas insertadas dentro de una prueba, que si se revierten) seguirían diciendo "ya instalado" aunque la tabla real ya no exista — un fallo real encontrado escribiendo las pruebas de `nd-search`, documentado en el docblock de `SearchIndexRepositoryTest`.

**FULLTEXT y el aislamiento transaccional de `WP_UnitTestCase`**: `WP_UnitTestCase` envuelve cada prueba en una transacción MySQL revertida al final, pero las filas insertadas sin `COMMIT` no son visibles para `MATCH() ... AGAINST()` de InnoDB (a diferencia de un `SELECT` normal, que sí ve los propios cambios sin confirmar). Las pruebas de `nd-search` que ejercitan el índice FULLTEXT confirman explícitamente sus cambios y limpian las filas a mano en lugar de depender del `ROLLBACK` automático — ver el docblock de `SearchIndexRepositoryTest` para el patrón completo, reutilizado también en `SearchQueryOverrideTest`.
