...
- Auditoría de seguridad de toda la superficie REST/admin (permisos, escapado de salida, nonces, redirecciones), no solo de las 6 páginas nuevas de esta versión: confirmado que `Route` (nd-core) es "fail-closed" por diseño — `permission_callback` deniega por defecto si no se pasa uno explícito, así que no hay forma de registrar accidentalmente una ruta pública; que ningún paquete de los 13 llama a `register_rest_route()` directamente (todos pasan por `Router`); y que ninguno usa `__return_true` como permiso. El escapado de salida ya lo fuerza WPCS en cada `composer check` (0 errores en los 13 paquetes). El único endpoint público sin capability check (`/nd-ads/click/{id}`) resuelve el destino en servidor a partir del ID de campaña, nunca de un parámetro de la petición — sin riesgo de open-redirect, ya documentado desde alpha.4.

### Fixed

- **`nd-core`/`nd-theme` (crítico)**: `nd-theme` nunca arrancaba en un sitio real — encontrado activando el tema por primera vez en esta sesión (toda la verificación previa en navegador fue contra páginas de `wp-admin`, independientes del tema activo; el front-end nunca se había comprobado con nd-theme realmente activo). Causa raíz: `Application::boot()` arrancaba en `plugins_loaded`, pero un tema solo puede añadirse al filtro `nd_core/providers` cuando WordPress carga su `functions.php`, lo cual ocurre DESPUÉS de `plugins_loaded` (entre `setup_theme` y `after_setup_theme`, confirmado en el `wp-settings.php` de WordPress core) — `Application::resolveProviderClasses()` leía ese filtro antes de que nd-theme tuviera ocasión de registrarse, así que `NDTheme\Providers\ThemeServiceProvider` nunca se incluía. Sus assets (CSS/JS de Vite), theme supports y menús jamás se encolaban en ningún sitio real desde alpha.2. Corregido moviendo el arranque a `after_setup_theme` (prioridad 5, dejando margen para que cualquier provider que enganche su propio callback a `after_setup_theme` durante `boot()` se recoja en el mismo ciclo de `do_action()` — comportamiento estándar de `WP_Hook` desde WordPress 4.7). Verificado en vivo: `dist/app.css`/`dist/app.js` ahora cargan (antes ni se solicitaban), el modo oscuro funciona, y los meta tags/JSON-LD/breadcrumbs de nd-seo (documentados como completos desde alpha.3) se confirmaron funcionando en un artículo real por primera vez.
- **`nd-theme`**: de paso, esa misma verificación reveló que `single.php` llama a `comments_template()` sin que el tema tuviera un `comments.php` — WordPress recurría a su renderizado heredado con un aviso de obsolescencia impreso en cada artículo. Se añade un `comments.php` mínimo y funcional (comentarios nativos de lectores, deliberadamente distintos de los comentarios internos editoriales de nd-workflow).
